...
class Kernel:
	...
	def mechanism(self, connection = False):
		...
		if self.userAgentC.active:
			...
			address = requests.get(ipSite)
		self.textInfo.text += "\n:: IP your network: %s" %address.text
		...
		try:
			if self.userAgentC.active and self.torProxieC.active:
				...
				address = requests.get(ipSite, proxies = proxie)
				connection = True 
		except:
		...
		if connection:
			...
			self.textInfo.text += ":: IP Tor network: %s" %address.text 
		try:
...
class ParserApp(App, Kernel, Parse):
	...
	def build(self):
		...
		self.textTag = TextInput(text = "", multiline = False, hint_text = "Tag", font_size = 17)

		self.textAttribute = TextInput(text = "", multiline = False, hint_text = "Attribute", font_size = 17)
